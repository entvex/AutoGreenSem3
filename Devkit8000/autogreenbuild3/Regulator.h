...
class Regulator
{
 public:
  ...
  void ControlData( SensorData drivhus_data)
  {
	...
	cout << "Statusmessage: Temperaturen er OK i drivhus\n";
	...
	uart->activateSensor("windowoff");
      ...
      {
	cout << "statusmessage: Der er lidt for varmt i drivhus\n";
	//OPEN WinDOW
	...
	uart->activateSensor("windowon");
      ...
      {
	cout << "Statusmessage: Der er MEGET for varmt i drivhus\n";
	SysMsg* monmsg = new SysMsg;
	...
	//close Window
	//vent off
	cout << " statusmessage: Der er for koldt i drivhus\n";		
	SysMsg* monmsg = new SysMsg;
